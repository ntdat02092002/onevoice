...
`PhraseTtsPolicy` mặc định dùng `emission_mode: final_utterance`, sau đó phát cụm 8–24 token theo ranh giới câu; `max_chunk_tokens` là hard maximum kể cả ở final. UI ghép các phrase cùng utterance trước khi autoplay. `stable_sentence` đợi một target sentence giống nhau qua số revision cấu hình, còn `stable_phrase` là mode realtime chủ động hơn. Hai field `final_only` và `sentence_boundary_only` vẫn được đọc khi `emission_mode: null` để tương thích config cũ. VAD semantic endpoint chỉ đóng waveform/utterance và không quyết định TTS stability.
...
`phrase_id` phải được copy nguyên từ `TtsRequest` sang `TtsUpdate`. TTS worker kiểm tra `source_revision` cả trước và sau synthesis để audio từ bản dịch cũ không lọt ra sau khi revision mới đã tới. Lỗi ở translation final hoặc TTS sẽ reset state downstream của utterance tương ứng.
